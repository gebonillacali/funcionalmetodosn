...
metodoBiseccion a b e1 e2 yi = if (checkError a b e1) || (abs (testFuncion c yi)) <= e2 then c:[] else if ((testFuncion a yi) * (testFuncion c yi)) > 0 then c:[] ++ (metodoBiseccion a c e1 e2 yi) else c:[] ++ (metodoBiseccion c b e1 e2 yi) where c = (calcularMedia a b)
